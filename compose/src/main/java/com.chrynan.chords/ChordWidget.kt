...
internal fun calculateTopStringLabels(
    ...
    chord.mutes.forEach {
        ...
        val y = getVerticalCenterTextPosition(size.drawingBounds.top + size.stringTopLabelBounds.height() / 2, viewModel.mutedText, stringLabelTextPaint)

        stringTopMarkerPositions.add(
                StringPosition(
                        text = viewModel.mutedText,
                        textX = x,
                        textY = y))
    }
...
internal fun calculateBottomStringLabels(
    ...
    if (viewModel.stringLabelState != StringLabelState.HIDE) {
        chart.stringLabels.forEach {
            ...
            if (label != null) {
                val x = size.chartBounds.left + (chart.stringCount - it.string) * size.stringDistance + (chart.stringCount - it.string) * size.stringSize
...
internal fun drawFretNumbers(
    ...
    if (showFretNumbers) {
        fretNumberPoints.forEachIndexed { index, point ->
            canvas.nativeCanvas.drawText((chart.fretStart + index).toString(), point.x, point.y, fretLabelTextPaint)
        }
    }
}
